...
import src.pointnet2.pointnet2_utils as pointnet2_utils
...
class PointnetSAModuleVotes(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor,
        ...
        Parameters
        ----------
        xyz : torch.Tensor[batch_size, num_votes, 3]
            (B, N, 3) tensor of the xyz coordinates of the features
        features : torch.Tensor[batch_size, vote_feature_dim, num_votes]
        ...
        xyz_flipped = xyz.transpose(1, 2).contiguous()
        # xyz_flipped is a tensor of size [batch_size, 3, num_votes]
        if inds is None:
            inds = pointnet2_utils.furthest_point_sample(xyz, self.npoint)
            # sample npoint on the seed_points
        else:
            assert (inds.shape[1] == self.npoint)
        new_xyz = pointnet2_utils.gather_operation(
            xyz_flipped, inds
        ).transpose(1, 2).contiguous() if self.npoint is not None else None
        # new_xyz is a tensor of size [batch_size, npoints, 3]
