...
def normalize_random_graph(A):
    if type(A).__module__ == 'numpy':
        Dl = np.sum(A, 1)
        # print('the content of the Dl:', Dl)
        num_virtual_nodes, num_key_points = A.shape
        reshape_graph = A
        for i in range(num_virtual_nodes):
            reshape_graph[i] = reshape_graph[i] / Dl[i]
        # print("the content of the normalized graph:", reshape_graph)
        return A
