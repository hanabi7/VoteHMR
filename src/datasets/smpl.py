...
class SMPL(nn.Module):
    ...
    def get_current_joints(self, verts):
        joint_x = torch.matmul(verts, self.J_regressor)
        joint_y = torch.matmul(verts, self.J_regressor)
        joint_z = torch.matmul(verts, self.J_regressor)
        joints = torch.stack([joint_x, joint_y, joint_z], dim=2)
        return joints